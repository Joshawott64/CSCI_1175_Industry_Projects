...
public class UnweightedGraph<V> implements Graph<V> {
  ...
  /** Return a List that contains all vertices in a cycle starting from u 
   */
  public List<Integer> getACycle() {
	  ...
	  while (verticeList.size() > 0) {
		  int v = verticeList.get(0);
		  ...
		  while (!stack.isEmpty()) {
			  ...
			  if (neighbors.get(x).size() == 0) {
				  ...
				  for (int i = neighbors.get(x).size(); i >= 0; i--) {
					  Edge e = neighbors.get((int) x).get(i);
					  if (!isVisited[e.v]) {
						  parent[e.v] = x;
						  stack.push(e.v);
						  isVisited[e.v] = true;
						  verticeList.remove(new Integer(e.v));
